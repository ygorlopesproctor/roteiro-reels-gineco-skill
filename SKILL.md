...
| 🆕 `references/swipe-raianne.md` | **v2.3** · estética de copy da peer Raianne destrinchada (20 reels) — anatomia + 4 exemplares + tabela-mapa |
| 🆕 `references/cross-anatomia.md` | **v2.3** · o esqueleto INVARIANTE entre Nayara×Elias×Raianne + os 5 slots que trocam por nicho + a fórmula destilada "molde Maman" |
...
| 🆕 `assets/mentor-elias/` | Top 20 do mentor Elias Maman (mapa + JSON bruto) + **3 transcrições** de talking-heads (`transcricoes/`) — lastro do playbook e da cross-anatomia |
| 🆕 `assets/peer-raianne/` | Top 20 da peer Raianne Badiani (mapa + métricas + JSON) + **20 transcrições completas** (`transcricoes/`) — lastro do bloco D + `swipe-raianne.md` |
